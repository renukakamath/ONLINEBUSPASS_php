...
if (isset($_POST['amount'])) {
	extract($_POST);
	echo$q="insert into fares values(null,'$sid','$eid','$amo','$pamo')";
	insert($q);
	alert('  Successfully');
...
<center>
	...
	<table class="table" style="width: 500px;color: black">
		...
		<?php 
         echo$q="SELECT * FROM `fares` f,`stops` s WHERE f.`starting_stop_id`=s.`stop_id` AND f.`ending_stop_id`=s.`stop_id` And s.`route_id`='$rid'";
         $res=select($q);
         $slno=1;
         foreach ($res as $row) {
         	?>
